...
def _eigene_partner_id(odoo: Any) -> Optional[int]:
    ...
    try:
        ...
        if partner:
            ...
            return (
                int(partner[0]) if isinstance(partner, (list, tuple)) else int(partner)
            )
    except Exception:
        return None
    return None
...
def to_odoo_utc_marked(value: datetime) -> str:
    """UTC datetime that still says so, for values crossing a write helper.

    ``localize_datetime_values`` converts naive strings from local time. A tool
    that has already converted must therefore hand on a value that carries its
    offset, or the second pass would shift it again. Odoo accepts the offset
    form and normalises it, so nothing downstream needs to know.
    """
...
# model speaks the user's local time ("move it to 10:00"), so a naive value
# handed straight to Odoo is read as UTC and comes back shifted by the offset -
# in Zurich that is +1h in winter and +2h in summer.
#
# termin_buchen converted its own start/stop from the beginning. Every other
# write path did not, so creating an appointment was correct while moving one
# silently landed two hours late. Converting here, at the single choke point
# both gated_create and gated_write pass through, fixes every tool at once and
# keeps the rule in one place.
def localize_datetime_values(
    odoo: Any,
    model: str,
    values: Dict[str, Any],
) -> Dict[str, Any]:
    """Convert naive datetime strings in ``values`` from local time to UTC.

    Which fields are datetimes is asked of Odoo rather than hardcoded, so this
    works for ``date_deadline``, ``date_order`` and any custom field without a
    list to maintain. ``date`` fields are left alone: a calendar day has no
    timezone and shifting it would move it to the wrong day.

    Values that already carry an offset (``...+02:00``, ``...Z``) are trusted
    as given. Anything unparseable is passed through untouched so a bad input
    fails in Odoo with a clear error instead of being silently rewritten.
    """
    if not values:
        return values
    try:
        fields = odoo.get_model_fields(model) or {}
    except Exception:  # noqa: BLE001 - never block a write over field metadata
        return values
    ...
    for name, value in values.items():
        ...
        try:
            ...
            continue  # let Odoo reject it with its own message
        out[name] = to_odoo_utc(parsed)
    return out
...
def register_pending(
    ctx: Any, instance_name: str, tool: str, payload: Dict[str, Any]
) -> str:
...
def take_pending(
    ctx: Any, instance_name: str, tool: str, code: Optional[str]
) -> Dict[str, Any]:
    ...
    with _PENDING_LOCK:
        ...
        if code is None or code not in _PENDING:
            candidates = [
                ...
                if item["tool"] == tool
                and item["instance"] == instance_name
            ...
            if len(candidates) == 1:
                ...
                raise ValueError(
                    "Keine offene Freigabe fuer diese Aktion - zuerst ohne bestaetigen aufrufen, Karte zeigen, dann bestaetigen."
                )
            else:
                raise ValueError(
                    "Mehrere offene Freigaben - bitte den freigabe_code aus der zuletzt gezeigten Karte angeben."
                )
        item = _PENDING.get(code)
        if item is None:
            raise ValueError(
                "Unbekannter oder bereits verwendeter freigabe_code - Vorschau wiederholen."
            )
        if item["expires_at"] < time.time():
            _PENDING.pop(code, None)
            raise ValueError(
                "Der freigabe_code ist abgelaufen (10 Minuten) - Vorschau wiederholen."
            )
        if item["tool"] != tool or item["instance"] != instance_name:
            raise ValueError(
                "Der freigabe_code gehoert zu einer anderen Aktion oder Instanz."
            )
        if item["binding"] != _identity_binding(ctx, instance_name):
            raise ValueError(
                "Der freigabe_code wurde von einem anderen Benutzer erzeugt - Vorschau als aktueller Benutzer wiederholen."
            )
        _PENDING.pop(code, None)
...
def _card(
    tool: str, aktion: str, details: Dict[str, Any], frage: str, code: str
) -> Dict[str, Any]:
    return {
        "success": True,
        "tool": tool,
...
def gated_create(
    ...
    *,
    ...
    core = _core()
    _, odoo = core._resolve_odoo(ctx, instance)
    values = localize_datetime_values(odoo, model, values)
    report = core.validate_write(
        ctx, model, "create", values=values, context=context, instance=instance
    )
    if not report.get("success") or not (report.get("approval_status") or {}).get(
        "stored"
    ):
        issues = report.get("issues") or []
        detail = "; ".join(
            str(issue.get("message")) for issue in issues if isinstance(issue, dict)
        ) or report.get("error")
...
def gated_write(
    ...
    *,
    ...
    values = localize_datetime_values(odoo, model, values)
    report = core.validate_write(
        ctx, model, "write", values=values, record_ids=record_ids, instance=instance
    )
    if not report.get("success") or not (report.get("approval_status") or {}).get(
        "stored"
    ):
...
def gated_unlink(
    ctx: Any, model: str, record_ids: List[int], *, instance: Optional[str] = None
) -> None:
    core = _core()
    report = core.validate_write(
        ctx, model, "unlink", record_ids=record_ids, instance=instance
    )
    if not report.get("success") or not (report.get("approval_status") or {}).get(
        "stored"
    ):
...
def gated_method(
    ...
    *,
    ...
    result = _core().execute_method(
        ctx, model, method, args=args, kwargs=kwargs, instance=instance
    )
    if not result.get("success"):
...
def fsm_person_id(
    ctx: Any, odoo: Any, name: str, *, instance: Optional[str] = None
) -> int:
    ...
    try:
        ...
        def day(offset: int) -> Dict[str, str]:
            ...
            return {
                ...
                "wochentag": WEEKDAYS_DE[d.weekday()],
            }
...
def termin_buchen(
    ...
    try:
        ...
        values: Dict[str, Any] = {
            "name": titel.strip(),
            "start": to_odoo_utc_marked(begin),
            "stop": to_odoo_utc_marked(end),
            "allday": False,
        }
        if beschreibung:
        ...
        anzeige = {
            ...
            "zeitzone": str(_local_tz().key),
            "odoo_utc": {"start": to_odoo_utc(begin), "stop": to_odoo_utc(end)},
        }
        if not bestaetigen:
            report = api.validate_write(
                ctx, "calendar.event", "create", values=values, instance=instance_name
            )
            ...
            code = register_pending(
                ctx, instance_name, tool, {"values": values, "anzeige": anzeige}
            )
            return _card(
                tool,
                "Termin ANLEGEN",
        ...
        event_id = gated_create(
            ctx, "calendar.event", payload["values"], instance=instance_name
        )
...
def create_partner(
    ...
    try:
        ...
        if not bestaetigen:
            ...
            report = api.validate_write(
                ctx, "res.partner", "create", values=values, instance=instance_name
            )
            if not report.get("success"):
                return _fail(
                    tool, "Kontakt-Daten ungueltig", issues=report.get("issues")
                )
            ...
            return _card(
                ...
                "Kontakt ANLEGEN",
                details,
                f"Kontakt '{values['name']}' ({art}) anlegen?",
                code,
            )
        payload = take_pending(ctx, instance_name, tool, freigabe_code)
        partner_id = gated_create(
            ctx, "res.partner", payload["values"], instance=instance_name
        )
        return {
            "success": True,
            "tool": tool,
            "partner_id": partner_id,
            "summary": f"Kontakt '{payload['values']['name']}' angelegt (ID {partner_id}).",
        }
    except Exception as exc:  # noqa: BLE001
        return _fail(tool, exc)
...
def get_account_by_code(
    ctx: Context, code: str, instance: Optional[str] = None
) -> Dict[str, Any]:
    ...
    try:
        ...
        rows = odoo.search_read(
            "account.account",
            [["code", "=like", str(code).strip() + "%"]],
            fields=["code", "name", "account_type"],
            limit=10,
        )
        rows, redacted = api.get_field_policy().redact_records(
            instance_name, "account.account", list(rows)
        )
        return {
            "success": True,
            "tool": tool,
...
def bericht_link(
    ...
    try:
        ...
        instance_name, odoo = api._resolve_odoo(ctx, instance)
        rows = odoo.search_read(
            model, [["id", "=", int(record_id)]], fields=["display_name"], limit=1
        )
        if not rows:
            raise ValueError(
                f"Kein Datensatz {record_id} in '{model}' gefunden - oder keine Leseberechtigung."
            )
        ...
        if report_name:
            chosen = next(
                (
                    r
                    for r in reports
                    if r["report_name"] == report_name or r["name"] == report_name
                ),
                None,
            )
            if chosen is None:
                raise ValueError(
                    f"Kein PDF-Bericht '{report_name}' fuer '{model}'. Verfuegbar: "
...
def auftrag_monteur_zuweisen(
    ...
    try:
        ...
        rows = odoo.read_records(
            "fsm.order", [int(order_id)], fields=["name", "person_id"]
        )
        if not rows:
        ...
        if not bestaetigen:
            ...
            return _card(
                ...
                "Monteur ZUTEILEN",
                {
                    "auftrag": rows[0].get("name"),
                    "id": int(order_id),
                    "vorher": current[1] if current else "keine Zuteilung",
                    "nachher": monteur,
                },
        ...
        gated_write(
            ...
            "fsm.order",
            [payload["order_id"]],
            {"person_id": person_id, "person_ids": [[4, person_id]]},
            instance=instance_name,
        )
        return {
            "success": True,
            ...
            "auftrag_id": payload["order_id"],
            "person_id": person_id,
            "summary": f"{payload['monteur']} ist dem Auftrag {payload['auftrag']} zugeteilt.",
        }
    except Exception as exc:  # noqa: BLE001
        return _fail(tool, exc)
...
def auftrag_abschliessen(
    ...
    try:
        ...
        instance_name, odoo = api._resolve_odoo(ctx, instance)
        rows = odoo.read_records(
            "fsm.order", [int(order_id)], fields=["name", "stage_id"]
        )
        if not rows:
            raise ValueError(f"Auftrag #{order_id} gibt es nicht.")
        ...
        if not bestaetigen:
            code = register_pending(
                ...
                instance_name,
                tool,
                {"order_id": int(order_id), "auftrag": rows[0].get("name")},
            )
            return _card(
                tool,
                "Auftrag ABSCHLIESSEN",
                {
                    "auftrag": rows[0].get("name"),
                    "id": int(order_id),
                    "stufe_vorher": stage[1] if stage else "-",
                    "stufe_nachher": "Completed",
                },
        ...
        after = odoo.read_records(
            "fsm.order", [payload["order_id"]], fields=["stage_id"]
        )
        ...
        return {
            ...
            "auftrag": payload["auftrag"],
            "auftrag_id": payload["order_id"],
            "stufe": new_stage[1] if new_stage else "?",
            "summary": f"Auftrag {payload['auftrag']} ist abgeschlossen.",
        }
    except Exception as exc:  # noqa: BLE001
        return _fail(tool, exc)
...
def _find_tax_id(odoo: Any, percent: float, move_type: str) -> Optional[int]:
    ...
    rows = odoo.search_read(
        "account.tax",
        [["type_tax_use", "=", use], ["amount", "=", percent], ["active", "=", True]],
        fields=["id"],
        limit=1,
    )
...
def create_invoice(
    ...
    try:
        ...
        if move_type not in ("out_invoice", "in_invoice", "out_refund"):
            raise ValueError(
                "move_type muss out_invoice, in_invoice oder out_refund sein."
            )
        if not lines:
        ...
        for line in lines:
            ...
            total += qty * price
            lvals: Dict[str, Any] = {
                "name": str(line.get("description", "Position")),
                "quantity": qty,
                "price_unit": price,
            }
            code = line.get("account_code")
            if code:
                acc = odoo.search_read(
                    "account.account",
                    [["code", "=like", str(code) + "%"]],
                    fields=["id"],
                    limit=1,
            ...
            if percent > 0:
                ...
                if tax_cache[percent]:
                    ...
                    tax_notes.add(
                        f"ACHTUNG: kein {percent}%-Steuersatz in Odoo gefunden - Zeile OHNE MwSt"
                    )
            else:
                lvals["tax_ids"] = [[6, 0, []]]
        ...
        if not bestaetigen:
            ...
            return _card(
                ...
                {
                    "partner": partner_name,
                    "betrag_chf": round(total, 2),
                    "zeilen": len(invoice_lines),
                    "mwst": sorted(tax_notes),
                    "schwelle_chf": chf_schwelle(),
                    "ueber_schwelle": total > chf_schwelle(),
                },
                f"Rechnung ueber CHF {total:,.2f} an {partner_name} als Entwurf anlegen?",
                code,
            )
        payload = take_pending(ctx, instance_name, tool, freigabe_code)
        move_id = gated_create(
            ctx,
            "account.move",
            payload["values"],
            instance=instance_name,
        ...
        info = odoo.read_records(
            "account.move", [move_id], fields=["name", "state", "amount_total"]
        )
        info0 = (
            info[0]
            if info
            else {"name": "/", "state": "draft", "amount_total": payload["total"]}
        )
        label = (
            info0["name"]
            if info0.get("name") and info0["name"] != "/"
            else f"#{move_id}"
        )
        return {
            "success": True,
...
def create_project(
    ...
    try:
        ...
        if not bestaetigen:
            existing = odoo.search_read(
                "project.project", [["name", "=", project_name]], fields=["id"], limit=3
            )
            code = register_pending(ctx, instance_name, tool, {"values": values})
            ...
            return _card(
                ...
                "Projekt ANLEGEN",
                details,
                f"Projekt '{project_name}' anlegen?",
                code,
            )
        payload = take_pending(ctx, instance_name, tool, freigabe_code)
        project_id = gated_create(
            ctx, "project.project", payload["values"], instance=instance_name
        )
        return {
            "success": True,
            "tool": tool,
            "project_id": project_id,
            "summary": f"Projekt '{payload['values']['name']}' angelegt (ID {project_id}).",
        }
    except Exception as exc:  # noqa: BLE001
        return _fail(tool, exc)
    ...
    try:
        ...
        if not bestaetigen:
            ...
            if resolved_project_id:
                rows = odoo.read_records(
                    "project.project", [int(resolved_project_id)], fields=["name"]
                )
                if not rows:
                ...
                if not rows:
                    raise ValueError(
                        f"Kein Projekt mit dem Namen '{project_name}' gefunden."
                    )
                if len(rows) > 1:
                    raise ValueError(
                        f"Mehrere Projekte mit dem Namen '{project_name}' gefunden (IDs {[r['id'] for r in rows]}) - bitte project_id angeben."
                    )
            ...
            code = register_pending(
                ...
                instance_name,
                tool,
                {"values": values, "projekt": resolved_project_name},
            )
            return _card(
                tool,
                "Aufgabe ANLEGEN",
                {
                    "titel": task_title,
                    "projekt": resolved_project_name,
                    "projekt_id": resolved_project_id,
                },
                f"Aufgabe '{task_title}' im Projekt '{resolved_project_name}' anlegen?",
                code,
            )
        payload = take_pending(ctx, instance_name, tool, freigabe_code)
        task_id = gated_create(
            ctx, "project.task", payload["values"], instance=instance_name
        )
        return {
            "success": True,
            "tool": tool,
            "task_id": task_id,
            "summary": f"Aufgabe '{payload['values']['name']}' im Projekt '{payload['projekt']}' angelegt (ID {task_id}).",
        }
    except Exception as exc:  # noqa: BLE001
        return _fail(tool, exc)
...
def create_quotation(
    ...
    try:
        ...
        payload = take_pending(ctx, instance_name, tool, freigabe_code)
        order_id = gated_create(
            ctx, "sale.order", payload["values"], instance=instance_name
        )
        info = odoo.read_records(
            "sale.order", [order_id], fields=["name", "amount_total"]
        )
        info0 = (
            info[0]
            if info
            else {"name": f"#{order_id}", "amount_total": payload["total"]}
        )
        return {
            "success": True,
            "tool": tool,
...
def update_record(
    ...
    try:
        ...
        return {
            ...
            "summary": f"'{payload['label']}' ({payload['model']}) aktualisiert.",
        }
    except Exception as exc:  # noqa: BLE001
        return _fail(tool, exc)
    ...
    try:
        ...
        if not bestaetigen:
            code = register_pending(
                ...
                instance_name,
                tool,
                {"model": model, "record_id": int(record_id), "label": label},
            )
            return _card(
                tool,
                "Datensatz LOESCHEN",
        ...
        payload = take_pending(ctx, instance_name, tool, freigabe_code)
        gated_unlink(
            ctx, payload["model"], [payload["record_id"]], instance=instance_name
        )
        return {
            "success": True,
            "tool": tool,
            "model": payload["model"],
            "record_id": payload["record_id"],
            "summary": f"'{payload['label']}' ({payload['model']}) geloescht.",
        }
    except Exception as exc:  # noqa: BLE001
        return _fail(tool, exc)
...
def post_journal_entry(
    ...
    try:
        ...
        rows = odoo.read_records(
            "account.move",
            [int(move_id)],
            fields=["name", "state", "amount_total", "partner_id", "move_type"],
        )
        if not rows:
        ...
        if info.get("state") == "posted":
            return {
                ...
                "move_id": int(move_id),
                "state": "posted",
                "summary": f"{info.get('name')} ist bereits gebucht.",
            }
        if not bestaetigen:
            code = register_pending(
                ctx,
                instance_name,
                tool,
                {"move_id": int(move_id), "name": info.get("name")},
            )
            return _card(
                tool,
                "Beleg BUCHEN",
                {
                    "beleg": info.get("name"),
                    "betrag_chf": info.get("amount_total"),
                    "partner": info["partner_id"][1] if info.get("partner_id") else "-",
                    "typ": info.get("move_type"),
                },
        ...
        after = odoo.read_records(
            "account.move", [payload["move_id"]], fields=["name", "state"]
        )
        ...
        return {
            ...
            "state": state,
            "summary": f"{(after[0].get('name') if after else payload['name'])} wurde gebucht.",
        }
    except Exception as exc:  # noqa: BLE001
        return _fail(tool, exc)
...
def pay_invoice(
    ...
    try:
        ...
        if info.get("state") != "posted":
            raise ValueError(
                f"{info.get('name')} ist nicht gebucht - erst buchen, dann zahlen."
            )
        if not bestaetigen:
            code = register_pending(
                ctx,
            ...
            return _card(
                ...
                {
                    "beleg": info.get("name"),
                    "offen_chf": info.get("amount_residual"),
                    "partner": info["partner_id"][1] if info.get("partner_id") else "-",
                },
        ...
        after = odoo.read_records(
            "account.move", [payload["move_id"]], fields=["name", "payment_state"]
        )
        return {
            "success": True,
            "tool": tool,
            "move_id": payload["move_id"],
            "payment_state": after[0].get("payment_state") if after else "?",
            "summary": f"Zahlung fuer {(after[0].get('name') if after else payload['name'])} registriert.",
        }
    except Exception as exc:  # noqa: BLE001
        return _fail(tool, exc)
    ...
    try:
        ...
        if not bestaetigen:
            ...
            return _card(
                ...
                {
                    "kunde": plan["kunde"].get("name"),
                    "standort": plan["standort"].get("name"),
                    "beschreibung": beschreibung,
                    "zeitzeilen": len(plan["zeiten"]),
                    "materialzeilen": len(plan["material"]),
                    "zuschlaege": plan["zuschlaege"],
                    "monteur": monteur
                    or next(
                        (z.get("monteur") for z in plan["zeiten"] if z.get("monteur")),
                        None,
                    ),
                },
        ...
        if loc:
            ...
            fresh = odoo.search_read(
                "fsm.location", [["id", "=", ort_id]], fields=["partner_id"], limit=1
            )
            if fresh and fresh[0].get("partner_id"):
                gated_write(
                    ctx,
                    "res.partner",
                    [int(fresh[0]["partner_id"][0])],
                    {"parent_id": kunde_id},
                    instance=instance_name,
                )
        ...
        monteur_name = plan.get("monteur") or next(
            (z.get("monteur") for z in plan["zeiten"] if z.get("monteur")), None
        )
        if monteur_name:
        ...
        for z in plan["zeiten"]:
            ...
            line = {
                "name": z.get("text") or z.get("typ") or "Arbeit",
                "employee_id": employee_id,
                "unit_amount": float(z.get("stunden") or 0),
                "date": z.get("datum") or date.today().isoformat(),
                "fsm_order_id": order_id,
            }
            if z.get("typ"):
                line["time_type_id"] = find_or_create(
                    ctx,
                    odoo,
                    "project.time.type",
                    [["name", "=", z["typ"]]],
        ...
        for m in plan["material"]:
            product_id = find_or_create(
                ...
                [["name", "=", m.get("produkt")]],
                {
                    "name": m.get("produkt"),
                    "type": "consu",
                    "list_price": float(m.get("preis") or 0),
                },
                instance=instance_name,
            )
            line = {
                ...
                "quantity": float(m.get("menge") or 1),
            }
            if m.get("edv_nr"):
        ...
        for tag in plan["zuschlaege"]:
            ...
            gated_create(
                ctx,
                "fsm.gav.line",
                {"order_id": order_id, "tag_id": tag_id},
                instance=instance_name,
            )
        if plan["zuschlaege"]:
...
READ_TOOLS: List[tuple[Callable[..., Any], str]] = [
    (
        aktuelles_datum,
        "Heutiges Datum/Uhrzeit in Schweizer Zeit mit Wochentag, morgen, uebermorgen - fuer relative Datumsangaben zuerst aufrufen",
    ),
    (
        get_account_by_code,
        "Konto im Schweizer KMU-Kontenrahmen nachschlagen (nur lesen)",
    ),
    (
        bericht_link,
        "Adresse des PDF-Berichts zu einem Datensatz liefern (nur lesen, kein Download)",
    ),
]
WRITE_TOOLS: List[tuple[Callable[..., Any], str]] = [
    (
        termin_buchen,
        "Kalender-Termin anlegen: 1. Aufruf = Karte + freigabe_code, 2. Aufruf mit bestaetigen=true + freigabe_code = anlegen. Zeiten in Schweizer Zeit.",
    ),
    (
        create_partner,
        "Kontakt (Kunde/Lieferant) anlegen: Karte, dann bestaetigen=true + freigabe_code.",
    ),
    (
        notiz_hinterlassen,
        "Interne Notiz im Chatter eines Datensatzes hinterlassen: Karte, dann bestaetigen=true + freigabe_code.",
    ),
    (create_project, "Projekt anlegen: Karte, dann bestaetigen=true + freigabe_code."),
    (
        create_task,
        "Aufgabe in einem Projekt anlegen: Karte, dann bestaetigen=true + freigabe_code.",
    ),
    (
        create_quotation,
        "Offerte/Angebot als Entwurf erstellen (CH-MwSt 8.1 % Standard): Karte, dann bestaetigen=true + freigabe_code.",
    ),
    (
        update_record,
        "Beliebigen bestehenden Datensatz aendern (Termin verschieben, Teilnehmer hinzufuegen, Aufgaben-Phase, Offerte/Rechnung-Felder usw.): Karte, dann bestaetigen=true + freigabe_code.",
    ),
    (
        delete_record,
        "Beliebigen bestehenden Datensatz unwiderruflich loeschen: Karte, dann bestaetigen=true + freigabe_code.",
    ),
    (
        auftrag_monteur_zuweisen,
        "Field-Service-Auftrag einem Monteur zuteilen: Karte, dann bestaetigen=true + freigabe_code.",
    ),
    (
        auftrag_abschliessen,
        "Field-Service-Auftrag abschliessen (action_complete): Karte, dann bestaetigen=true + freigabe_code.",
    ),
    (
        create_invoice,
        "Rechnung als Entwurf anlegen (CH-MwSt 8.1 % Standard): Karte, dann bestaetigen=true + freigabe_code.",
    ),
    (
        post_journal_entry,
        "Beleg/Rechnung buchen (action_post): Karte, dann bestaetigen=true + freigabe_code.",
    ),
    (
        pay_invoice,
        "Zahlung fuer eine gebuchte Rechnung registrieren: Karte, dann bestaetigen=true + freigabe_code.",
    ),
    (
        einsatzrapport_erstellen,
        "Kompletten Field-Service-Einsatz anlegen (Kunde, Standort, Auftrag, Zeiten, Material, Zuschlaege): Karte, dann bestaetigen=true + freigabe_code.",
    ),
]
